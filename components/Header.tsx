 ...
 * - Animated active link underline indicator
 * - High-fidelity full-height interactions
 * - Defined border highlight design
 * - Responsive navigation with Google Translate
 * - Only renders links for pages that actually exist in the repo
 * - Link Audit: All NAV_LINKS verified as valid routes
 */
export default function Header() {
  ...
  // Desktop navigation link classes — dynamic based on active state
  const getNavLinkClasses = (href: string) => {
    const active = isActive(href);
    return [
      'relative px-5 h-full flex items-center text-sm font-semibold tracking-wide transition-all duration-300',
  ...
  // Mobile navigation link classes — dynamic based on active state
  const getMobileNavLinkClasses = (href: string) => {
    const active = isActive(href);
    return [
      'text-2xl font-bold py-2 block border-b border-white/10',
  ...
  return (
    ...
    >
              ...
              return (
                <li role="none" key={link.name} className="relative h-full flex">
                  ...
                  >
                    ...
                    {active && (
                      <span
                        className="absolute bottom-0 left-3 right-3 h-0.5 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full"
                        style={{
                ...
                <li role="none" key={link.name}>
                  ...
                  >
                    <span className="flex items-center gap-3">
                      {active && (
                        <span className="inline-block w-1.5 h-1.5 rounded-full bg-gradient-to-r from-blue-400 to-purple-400 flex-shrink-0" />
                      )}
                      {link.name}
                    </span>
                  </Link>
      ...
      {/* Keyframe for active indicator fade-in */}
      <style jsx global>{`
